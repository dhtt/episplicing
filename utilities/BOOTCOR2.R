...
all_pairs.exp = readRDS("server_con/all_pairs.exp.RDS")
head(all_pairs.exp)
...
#     pair.his = pair.his %>%
#       mutate(m_val = if_else(!is.na(V10), 
...
pearcor_p <- function(exp, his){
  n = length(exp)
  if (length(unique(exp)) > 1 & length(unique(his)) > 1 & n >= 3){
    p_val = p_value_calculator(cor(exp, his, method = "pearson"), n)
